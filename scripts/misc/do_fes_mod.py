...
# read KBT_
KBT_ = float(sys.argv[3*NCV_+5])
OUTF_ = sys.argv[3*NCV_+6]
...
for i,lines in enumerate(open(FILENAME_, "r").readlines()):
    if (i+SHIFT)%NUM!=0: continue
    riga = lines.strip().split()
    # check format
    if(len(riga)!=NCV_+1 and len(riga)!=NCV_+2):
      print(FILENAME_,"is in the wrong format!")
      exit()
    # read CVs
    cvs = []
    for i in range(1, NCV_+1):
